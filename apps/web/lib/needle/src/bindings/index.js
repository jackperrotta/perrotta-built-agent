...
export async function getUsdModule(opts) {
    ...
    // Simplified loader for non-Vite environments (static server)
    const baseUrl = import.meta.url.substring(0, import.meta.url.lastIndexOf('/') + 1);

    const bindings = { default: baseUrl + "emHdBindings.js" };
    const data = { default: baseUrl + "emHdBindings.data" };
    // Assuming simple worker is fine
    const worker = { default: baseUrl + "emHdBindings.worker.js" };
    const wasm = { default: baseUrl + "emHdBindings.wasm" };

    const preloaded_data = await fetch(data.default).then(r => r.arrayBuffer());

    return usd_module_promise = getUsdModuleFn({
        mainScriptUrlOrBlob: bindings.default,
        ...opts,
        setStatus: (status) => {
